...
const Dashboard = () => {
  const { connected, address, role, connect, disconnect, publicKeyBase64, secretKey } = useWallet();
              ...
              >
                ...
                <div className="relative p-8 md:p-12 rounded-3xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10 hover:border-white/20 transition-all">
                  {role === 'employer' ? (
                    ...
                    <div>
                      <div className="mb-8">
                        <p className="text-slate-400 text-sm mb-2">This Month's Salary</p>
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  {(role === 'employer' ? employerEntries : employeeEntries).map((tx) => {
                    ...
                    const counterparty = isSent ? tx.employeeWallet : tx.transaction?.from;
                    return (
                      <div key={tx.id} className="flex items-center justify-between p-4 rounded-lg hover:bg-white/5 transition-all group cursor-pointer">
                        <div className="flex items-center gap-4">
                          <div className={`p-3 rounded-lg ${isSent ? 'bg-red-500/20' : 'bg-green-500/20'}`}> 
                            {isSent ? (
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-400">{period ? period : date}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
                          <p className="font-semibold">{isSent ? '-' : '+'}{amount}</p>
